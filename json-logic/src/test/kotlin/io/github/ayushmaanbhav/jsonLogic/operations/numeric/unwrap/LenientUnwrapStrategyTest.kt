...
class LenientUnwrapStrategyTest : BehaviorSpec({
    ...
    given("A false value") {
        ...
        `when`("unwrapped") {
            val unwrapResult = strategyImplementation.unwrapValueAsBigDecimal(config, wrappedValue)
            then("should be equal to 0") {
                unwrapResult shouldHaveSize 1
                unwrapResult.first() shouldBe 0.0.toBigDecimalDefaultContext()
            }
        }
    }
    ...
    given("A true value") {
        ...
        `when`("unwrapped") {
            ...
            then("should be equal to 1") {
                unwrapResult shouldHaveSize 1
                unwrapResult.first() shouldBe 1.0.toBigDecimalDefaultContext()
            }
        }
    }
    ...
    given("An empty list") {
        ...
        `when`("unwrapped") {
            ...
            then("should be equal to 0") {
                unwrapResult shouldHaveSize 1
                unwrapResult.first() shouldBe 0.toBigDecimalDefaultContext()
            }
        }
    }
