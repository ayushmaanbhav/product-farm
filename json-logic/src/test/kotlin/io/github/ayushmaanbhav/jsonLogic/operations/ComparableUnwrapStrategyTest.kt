...
class ComparableUnwrapStrategyTest : BehaviorSpec({
    ...
    given("Two numbers") {
        ...
        `when`("unwrapped") {
            ...
            then("should have the same double values") {
                unwrapResult shouldBe listOf(BigDecimal(firstWrappedValue.toString()), BigDecimal(secondWrappedValue.toString()))
            }
        }
    }
    ...
    given("Number and any string") {
        ...
        `when`("unwrapped") {
            ...
            then("should be number and null") {
                unwrapResult shouldBe listOf(BigDecimal(firstWrappedValue.toString()), null)
            }
        }
    }
    ...
    given("Number and number string") {
        ...
        `when`("unwrapped") {
            val unwrapResult = strategyImplementation.unwrapAsComparable(firstWrappedValue, secondWrappedValue)
            then("should have the same double values") {
                unwrapResult shouldBe listOf(BigDecimal(firstWrappedValue.toString()), BigDecimal(secondWrappedValue))
            }
        }
    }
