...
object JoinToString : StandardLogicOperation {
    ...
    override fun evaluateLogic(config: StandardLogicOperationConfig, expression: Any?, data: Any?): Any? = expression.asList.toOperationArguments()?.join()
    ...
    private fun List<Any?>.checkLimitArg() = (get(LIMIT_ARG_INDEX) as Number).takeIf { it.isWhole() }?.toInt()
